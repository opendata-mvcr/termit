...
//
//                conn.getStatements(null, RDF.TYPE, vf.createIRI(Vocabulary.s_c_slovnik)).stream().filter( s ->
//                        s.getSubject().stringValue().contains(VOCABULARY_IRI.toString())
//                ).map( s -> s.getContext() );
//
...
//    }
    ...
    void importResolvesVocabularyIriForContextWhenMultipleStreamsWithGlossaryAndVocabularyAreProvided() {
        ...
        transactional(() -> {
            final Repository repo = em.unwrap(Repository.class);
            try (final RepositoryConnection conn = repo.getConnection()) {
                conn.getStatements(null, RDF.TYPE, vf.createIRI(Vocabulary.s_c_slovnik)).forEach( s ->
                        assertThat(s.getSubject().stringValue(), containsString(VOCABULARY_IRI.toString()))
                );
            }
        });
    }
