...
class BaseDaoTest extends BaseDaoTestRunner {
    ...
    void updateReturnsManagedInstance() {
        ...
        term.getLabel().set(Environment.LANGUAGE, labelUpdate);
        transactional(() -> {
            final Term updated = sut.update(term);
            assertTrue(em.contains(updated));
            assertEquals(labelUpdate, updated.getLabel().get(Environment.LANGUAGE));
        });
        assertEquals(labelUpdate, em.find(Term.class, term.getUri()).getLabel().get(Environment.LANGUAGE));
    }
